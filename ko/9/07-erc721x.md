...
ERC721 대신 ERC721x를 사용하면 꽤 큰 이점이 있다네 — 토큰의 배치 생성, 전송 배치 및 같은 컨트랙트 내의 다중 토큰 클래스들과 같은 것들 말이네.

좀비 배틀그라운드에서는 배치 생성 및 배치 전송이 가능해지면서, 우리의 초기 후원자들에게 카드를 처음 전달했을 때 가스비를 1,000배 절약했다네 😮
...
구현에 있어서 주요 차이점은 NFT는 각각 고유한 tokenId를 가지지만, FT는 그렇지 않다는 것이네. FT는 서로 교환이 가능해서 각각 고유한 토큰 ID를 주게 되면 배치 전송과 같은 것에 가스 비용이 더 많이 들어갈 수 있기 때문이지.
...
1. `uint => uint`인 `mapping` 타입의 `internal` 속성 변수, `nftTokenIdToMouldId`를 선언하게.
...
2. 새로운 NFT 토큰 ID를 생성할 방법이 필요하다네. mapping 변수 아래에 `nftTokenIdIndex`라는 이름을 가진 `uint` 타입의 변수를 선언하고 `1000000`으로 초기화하게.
...
3. 우리의 컨트랙트 아래쪽에 `uint _tokenId`와 `uint _amount`를 인자로 가지는 `convertToNFT`라는 이름의 `function`을 선언하게. `public` 속성이어야 하네.

4. 함수의 첫 부분에는 두 개의 `require` 구문이 있어야 하네. 먼저, 우리가 변환할 이 토큰이 NFT가 아닌 FT라는 것을 require할 것이네. 이것은 ERC721XToken.sol에 있는 `require(tokenType[_tokenId] == FT);` 코드로 실행할 수 있다네.

5. 두 번째로, `msg.sender`가 `_amount` 이상의 `_tokenId` 잔액을 가졌는지 확인하는 require 구문이 필요하다네. 이것은 우리가 `awardToken`에서 사용했던 `require` 구문과 같은 것이네 — 하지만, 이번에는 `"You do not own enough tokens"`라는 에러 메시지를 출력할 것이네.

    >주의: 우리의 정답 확인 기능은 간단하다네. 그래서 `_amount`가 먼저 올 경우에만 정답으로 인정될 것이네.
