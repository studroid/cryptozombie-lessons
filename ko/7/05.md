...
첫 번째 예제에서, 우리가 플레이어1의 상태 값을 변경하고 있다는 점을 쉽게 알 수 있지. 반면에 두 번째 예제에서는, 코드를 읽는 사람이 `0`이 무엇을 의미하는지 알려면 함수 정의를 찾아봐야 할 것이네.

## 좀비 배틀그라운드 플레이어 상태 값 바꾸기

위의 `changes.changePlayerDefense()`에 더해, 게임에서 다른 상태 값들을 변형하기 위해 `changes` 객체에서 우리가 쓸 수 있는 다른 메소드들이 있네:
